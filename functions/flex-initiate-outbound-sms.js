...
exports.handler = async function(context, event, callback) {
  ...
  const sendMessageViaChatChannel = async (chatChannelSid) => {
    ...
    let chatMessage;
    console.debug(`Sending the message '${messageBody}' from fromNumber '${fromNumber}' to toNumber '${toNumber}' using Chat Channel SID '${proxySessionSid}'`); 
    
    try {
      chatMessage = await client.chat.services(chatServiceSid)
                  .channels(chatChannelSid)
                  .messages
                  .create({body: messageBody}); // From defaults to "system"
    } catch (error) {
